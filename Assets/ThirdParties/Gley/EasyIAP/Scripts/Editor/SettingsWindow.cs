                    ...
                    if (productFoldouts[i])
                    {
                        ...
                        if (localShopProducts[i].value != null)
                        {
                            ...
                            for (int j = 0; j < itemCount; j++)
                            {
                                ...
                                int selectedIndex = Array.IndexOf(HelperCreator.ShopTypes, currentType);
                                if (selectedIndex < 0) selectedIndex = 0;

                                string[] displayedOptions = HelperCreator.ShopTypes.Select(t => t.ToString()).ToArray();
                                selectedIndex = EditorGUILayout.Popup(j + ": Reward Type:", selectedIndex, displayedOptions);

                                var curType = HelperCreator.ShopTypes[selectedIndex];
                                item.SetDataType(curType);

                                if (HelperCreator.ShopQuantityTypes.Any(x => x == curType))
                                {
                                    if (HelperCreator.ShopQuantityTime.Any(x => x == curType))
                                    {
